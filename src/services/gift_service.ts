...
export async function getGiftIncomeByRoomidPeriod(roomid: number, period: string) {
    const sql = `
    SELECT 
    t1.roomid, 
    SUM(t1.gift_price * t1.amount * t1.combo) / 1000 AS total
...
WHERE 
    t1.roomid = ?
    AND t1.coin_type = 'gold'
    AND t1.created_at >= NOW() - INTERVAL ? DAY
GROUP BY 
    t1.roomid;
    `
    const [rows] = await pool.query(sql, [roomid, period, roomid, period]);
    const result = rows as mysql.RowDataPacket[];
    return result[0].total;
}
